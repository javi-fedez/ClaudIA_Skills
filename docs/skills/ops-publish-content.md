...
| Contenedor | `root-files-1` (nginx:alpine) |
| Directorio VPS | `/workspace/ClaudIA_Agent/docs/public/` |
| Path URL protegido | `/TU_TOKEN_DE_ACCESO/` |
...
```
https://files.example.com/TU_TOKEN_DE_ACCESO/<nombre-del-archivo>
```
...
`/usr/share/nginx/html/TU_TOKEN_DE_ACCESO/`, por lo que el archivo
...
| `000` | Docker caído | `docker ps` y comprobar `root-files-1` |
...
ls /workspace/ClaudIA_Agent/docs/output/20260316_framework-consultoria-marca-b.html

# 2. Copiar al directorio público
cp /workspace/ClaudIA_Agent/docs/output/20260316_framework-consultoria-marca-b.html \
   /workspace/ClaudIA_Agent/docs/public/

# 3. Verificar HTTP 200
curl -sk -o /dev/null -w "%{http_code}" \
  "https://files.example.com/TU_TOKEN_DE_ACCESO/20260316_framework-consultoria-marca-b.html"
...
```
https://files.example.com/TU_TOKEN_DE_ACCESO/
```
...
- El path `/TU_TOKEN_DE_ACCESO/` actúa como token de acceso — no publicar esta URL en repositorios públicos ni documentos compartidos abiertamente.
